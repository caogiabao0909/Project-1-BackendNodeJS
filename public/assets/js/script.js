...
if (boxFilter) {
  ...
  buttonApply.addEventListener("click", () => {
    ...
    listFilter.forEach(name => {
      const inputElement = boxFilter.querySelector(`[name="${name}"]`);
...
if (formSearch) {
  ...
  formSearch.addEventListener("submit", (event) => {
    ...
    const stockBaby = parseInt(formSearch.querySelector("[stock-baby]").innerHTML, 10);
    if (stockBaby > 0) {
...
if (boxTourDetail) {
  ...
  const drawBoxDetail = () => {
    ...
    const quantityChilren = parseInt(inputStockChildren.value)
    ...
    stockChildren.innerHTML = quantityChilren;
    ...
    const totalPrice = (priceAdult * quantityAdult) + (priceChildren * quantityChilren) + (priceBaby * quantityBaby)
  ...
  buttonAddToCart.addEventListener("click", () => {
    ...
    if (quantityAdult > 0 || quantityChilren > 0 || quantityBaby > 0) {
      const cartItem = {
        tourId,
        quantityAdult,
        quantityChilren,
