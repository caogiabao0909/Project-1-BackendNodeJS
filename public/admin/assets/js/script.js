...
if (revenueChart) {
  ...
  const drawChart = (date) => {
    ...
    const days = daysInCurrentMonth > daysInPreviousMonth ? daysInCurrentMonth : daysInPreviousMonth
    const arrayDay = []
...
// Button Delete
const listButtonDelte = document.querySelectorAll("[button-delete]");
if (listButtonDelte.length > 0) {
  listButtonDelte.forEach(button => {
...
if (search) {
  ...
  if (valueCurrent) {
    search.value = valueCurrent.trim();
  }
}
// End Search
